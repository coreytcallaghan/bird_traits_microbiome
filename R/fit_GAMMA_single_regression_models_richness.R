...
single_regression_model_function <- function(predictor_name){
  ...
  temp_dat_with_big_birds <- analysis_dat %>%
    dplyr::select(ebird_COMMON_NAME, Sample_type, gamma_Richness, DOI, predictor_name) %>%
    rename(predictor=5) %>%
    dplyr::filter(complete.cases(.))
  
  if(predictor_name %in% c("Mass", "Range.Size", "mean_flock_size", "habitat_breadth", "pop_abund")) {
    
    mod_with <- brms::brm(gamma_Richness ~ log10(predictor) + (1|DOI/Sample_type),
                          family=gaussian(),
    ...
    mod_with <- brms::brm(gamma_Richness ~ predictor + (1|DOI/Sample_type),
                          family=gaussian(),
                          data=temp_dat_with_big_birds,
                          warmup=1000,
                          iter=4000, 
