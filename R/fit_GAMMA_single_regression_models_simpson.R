...
single_regression_model_function <- function(predictor_name){
  ...
  if(predictor_name %in% c("Mass", "Range.Size", "mean_flock_size", "habitat_breadth", "pop_abund")) {
    ...
    mod_with <- brms::brm(gamma_ISimpson ~ log10(predictor) + (1|DOI),
                          ...
                          file=paste0("model_objects/gamma_single_regression_ISimpson_", predictor_name, ".RDS"))
    ...
    saveRDS(draws, paste0("intermediate_results/gamma_single_regression_ISimpson_draws_", predictor_name, ".RDS"))
    ...
    saveRDS(fe_only, paste0("intermediate_results/gamma_single_regression_ISimpson_fe_only_", predictor_name, ".RDS"))
    
    fe_only_mean <- fe_only %>% 
      group_by(predictor) %>%
      summarize(.value = mean(.value))
    
    saveRDS(fe_only_mean, paste0("intermediate_results/gamma_single_regression_ISimpson_fe_only_mean_", predictor_name, ".RDS"))
    ...
    saveRDS(out_df, paste0("intermediate_results/single_regression_ISimpson_summary_", predictor_name, ".RDS"))
    ...
    saveRDS(out_df, paste0("intermediate_results/gamma_single_regression_ISimpson_summary_", predictor_name, ".RDS"))
