...
model_dat <- analysis_dat %>%
  dplyr::select(ebird_COMMON_NAME, Richness, ISimpson, DOI, Sample_type,
...
mod_with <- brms::brm(log10(ISimpson) ~ log10(Mass) + log10(Range.Size) + log10(habitat_breadth) +
                      ...
                      file=paste0("model_objects/multiple_regression_simpson.RDS"))
...
saveRDS(draws, paste0("intermediate_results/multiple_regression_simpson_draws.RDS"))
...
saveRDS(out_df, paste0("intermediate_results/multiple_regression_simpson_summary.RDS"))
