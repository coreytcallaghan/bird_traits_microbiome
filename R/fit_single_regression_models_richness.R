...
single_regression_model_function <- function(predictor_name){
  ...
  temp_dat_with_big_birds <- analysis_dat %>%
   ...
   if(predictor_name %in% c("Mass", "Range.Size", "mean_flock_size", "habitat_breadth", "pop_abund")) {
     ...
     mod_with <- brms::brm(Richness ~ log10(predictor) + (1|DOI/Sample_type) + (1|ebird_COMMON_NAME),
              family=poisson(),
              ...
              file=paste0("model_objects/single_regression_richness_", predictor_name, ".RDS"))
    ...
    saveRDS(draws, paste0("intermediate_results/single_regression_richness_draws_", predictor_name, ".RDS"))
    ...
    mod_with <- brms::brm(Richness ~ predictor + (1|DOI/Sample_type) + (1|ebird_COMMON_NAME),
              family=poisson(),
              data=temp_dat_with_big_birds,
              warmup=1000,
              iter=4000, 
