...
single_regression_model_function <- function(predictor_name){
  ...
  if(predictor_name %in% c("Mass", "Range.Size", "mean_flock_size", "habitat_breadth", "pop_abund")) {
    ...
    mod_with <- brms::brm(log10(ISimpson) ~ log10(predictor) + (1|DOI/Sample_type) + (1|ebird_COMMON_NAME),
                          family=gaussian(),
                          ...
                          file=paste0("model_objects/single_regression_ISimpson_", predictor_name, ".RDS"))
    ...
    saveRDS(draws, paste0("intermediate_results/single_regression_ISimpson_draws_", predictor_name, ".RDS"))
    ...
    saveRDS(fe_only_mean, paste0("intermediate_results/single_regression_ISimpson_fe_only_mean_", predictor_name, ".RDS"))
    ...
    mod_with <- brms::brm(log10(ISimpson) ~ predictor + (1|DOI/Sample_type) + (1|ebird_COMMON_NAME),
                          family=gaussian(),
                          data=temp_dat_with_big_birds,
                          warmup=1000,
                          iter=4000, 
